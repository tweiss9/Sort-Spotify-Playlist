...
def release_date():
    ...
    if is_new == True:
        ...
        for i in range(0, len(playlist_tracks), batch_size):
            ...
            sp.playlist_remove_all_occurrences_of_items(playlist_id=playlistId, items=batch_uris)
        ...
        for i in range(0, len(temp_playlist_tracks), batch_size):
            ...
            sp.playlist_add_items(playlist_id=playlistId, items=batch_uris)

        # Unfollow and delete the temporary playlist
        sp.current_user_unfollow_playlist(playlist_id=temp_playlist_id)
        sp.playlist_unfollow(playlist_id=temp_playlist_id)
        print("Done!")
